...
namespace vApus.Monitor {
    public class MonitorControl : DataGridView {
        ...
        public void Init(Entities wdyh) {
            Rows.Clear();
            Columns.Clear();

            _toDisplayRows.Clear();

            _monitorResultCache = new MonitorResult() { Monitor = Monitor.MonitorSourceName };
        ...
        public void AddCounters(Entities counters, string decimalSeparator) {
            try {
                ...
                var parent = FindForm() as Main;
                if (parent == null)
                    Loggers.Log(Level.Error, "Failed adding monitor values.", ex);
                else
                    Loggers.Log(Level.Error, "Failed adding monitor values.", ex, new object[] { Monitor.MonitorSourceName });
            }
        }
        ...
        public void Save(string fileName) {
            try {
                ...
                using (var sw = new StreamWriter(fileName, false)) {
                    sw.WriteLine(string.Join("\t", _monitorResultCache.Headers));
                    foreach (var row in newCache) sw.WriteLine(string.Join("\t", row));
                    sw.Flush();
                }
        ...
        public void SaveFiltered(string fileName) {
            try {
                List<string[]> newCache = GetSaveableCache();
                using (var sw = new StreamWriter(fileName, false)) {
                    sw.WriteLine(string.Join("\t", FilterArray(_monitorResultCache.Headers)));
                    foreach (var row in newCache) sw.WriteLine(string.Join("\t", FilterArray(row)));
                    sw.Flush();
                }
            } catch { MessageBox.Show("Cannot access '" + fileName + "' because it is in use!", string.Empty, MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }

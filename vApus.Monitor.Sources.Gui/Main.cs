...
namespace vApus.Monitor {
    ...
    public partial class Main : Form {
        ...
        private void InitMonitorView() {
            ...
            _previousMonitorSourceForParameters = Monitor.MonitorSource;
            _previousFilter = string.Join(", ", Monitor.Filter);
        ...
        private Exception InitMonitorSourceClient() {
            ...
            try {
                ...
                cboMonitorSourceClient.SelectedIndex = Monitor.MonitorSourceIndex;
            }
            catch (Exception ex) {
                exception = ex;
            }
        ...
        private void cboMonitorSourceClient_SelectedIndexChanged(object sender, EventArgs e) {
            Monitor.MonitorSource = cboMonitorSourceClient.SelectedItem as MonitorSourceClient;
            _monitorSourceClient = ClientFactory.Create(Monitor.MonitorSource.Type);
        ...
        private void _monitorSourceClient_OnMonitor(object sender, OnMonitorEventArgs e) {
            SynchronizationContextWrapper.SynchronizationContext.Send(delegate {
                try {
                    ...
                    Loggers.Log(Level.Error, "Monitor proxy on monitor failed.", ex, new object[] { Monitor.MonitorSourceName });
                }
            }, null);
        }
        ...
        async private void ConnectAndGetCounters() {
            ...
            if (Monitor.PreviousMonitorSourceIndexForCounters != Monitor.MonitorSourceIndex) {
                Monitor.PreviousMonitorSourceIndexForCounters = Monitor.MonitorSourceIndex;
                //Clear this when a new is selected.
                Monitor.Wiw.GetSubs().Clear();
            }
            lblMonitorSourceMismatch.Visible = false;

            btnGetCounters.Enabled = false;
            cboMonitorSourceClient.Enabled = false;
        ...
        private void __WDYH() {
            SynchronizationContextWrapper.SynchronizationContext.Send((state) => { InitMonitorSourceClient(); }, null);
            ...
            SynchronizationContextWrapper.SynchronizationContext.Send(delegate {
                ...
                cboMonitorSourceClient.Enabled = true;
        ...
        private void SetValuesToParameters() {
            ...
            for (int i = 0; i != _monitorSourceClient.Parameters.Length; i++) {
                if (i >= Monitor.ParameterValues.Length) break;

                Parameter parameter = _monitorSourceClient.Parameters[i];

                object candidate = Monitor.ParameterValues[i];
        ...
        private void StoreParameterValues() {
            ...
            Monitor.ParameterValues = parameterValues;
        }
        ...
        private void ExtractWIWForListViewAction() {
            ...
            try {
                ...
                if (!selectedChecked) {
                    Entity entity = Monitor.Wiw.GetEntity(selected.SubItems[1].Text);
                    Monitor.Wiw.GetSubs().Remove(entity);
                }
                tvwCounters.AfterCheck += tvwCounter_AfterCheck;

                SetChosenCountersInListViewItems();
                btnStart.Enabled = btnSchedule.Enabled = lvwEntities.Items.Count != 0 && Monitor.Wiw.GetSubs().Count != 0;
            }
            catch {
                throw;
            }
        ...
        private void ApplyToWIW(TreeNode counterNode) {
            ...
            Entity entity = Monitor.Wiw.GetEntity(entityName);
            if (entity == null) {
                entity = new Entity(entityName, true);
                Monitor.Wiw.GetSubs().Add(entity);
            }
            ...
            if (lvwiEntity.Checked) {
                if (Monitor.Wiw.GetSubs().Contains(entity)) {
            ...
            Monitor.Wiw.GetSubs().Sort(EntityComparer.GetInstance());
            foreach (var e in Monitor.Wiw.GetSubs()) {
        ...
        private void SetColumnFilter() {
            ...
            txtFilterMonitorControlColumns.Text = String.Join(", ", filter);
        }
        ...
        private void FillCounters() {
            if (lvwEntities.SelectedItems.Count != 0) {
                ...
                tvwCounters.Nodes.AddRange(FilterCounters(Monitor.Filter, selected.Tag as TreeNode[]));
                tvwCounters.AfterCheck += tvwCounter_AfterCheck;
                lvwEntities.ItemChecked += lvwEntities_ItemChecked;
        ...
        private void PushSavedWiW() {
            ...
            btnStart.Enabled =
                btnSchedule.Enabled =
                (Monitor.MonitorSourceIndex == Monitor.PreviousMonitorSourceIndexForCounters ||
                 lvwEntities.Items.Count == 0)
                    ? Monitor.Wiw.GetSubs().Count != 0
                    : false;
        ...
        private void SetFilter() {
            ...
            Monitor.Filter = l.ToArray();

            SetFilterTextBox();
        }
        private void SetFilterTextBox() {
            try {
                txtFilter.Text = string.Join(", ", Monitor.Filter);
                llblUncheckAllVisible.Enabled = HasCheckedNodes();
                llblCheckAllVisible.Enabled = HasUncheckedNodes();
            }
            catch {
        ...
        private bool CanPublish() {
            return true;
#warning publish
            //            return Publisher.Settings.PublisherEnabled && _monitorSourceClient != null; 
        }
        ...
        private bool StartMonitor() {
            ...
            try {
                ...
                if (_monitorSourceClient.Connect()) {
                    ...
                    _monitorSourceClient.WIW = Monitor.Wiw;

                    monitorControl.Init(_wdyh);
                ...
                Loggers.Log(Level.Error, message, ex, new object[] { Monitor.MonitorSourceName });
